...
const index = () => {
  ...
  const [itineraryFields, setItineraryFields] = useState([{
    name: '', photos: [], details: '', duration: '', location: { place: '', address: '', longitude: '', latitude: '', zoom: '' },
  }]);
...
//       setTour(toursData.find((item) => item.id == id));
  ...
  const handleItineraryRemoveImage = (index, imageIndex) => {
    ...
    const tlist = [...itineraryFields];
              tlist[index] = { 'photos': itineraryImages[index] };
              setItineraryFields(tlist);
  };

  const handleItineraryFileUpload = (even) => {
